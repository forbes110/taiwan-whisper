...
source path.sh
cd $root/training
# for 4 gpu 2 gradient accumulation
accelerate launch --num_processes 4 --main_process_port 29500 run_distillation.py \
  ...
  --output_dir "./output/hallucination/6gram-5repeat_base-per0.6_allow-empty" \
  --wandb_name "hallucination_6gram-5repeat_base-per0.6_allow-empty" \
  --do_train \
  --do_eval \
